...
( function() {
	...
	var template = '<img alt="" src=""  class=""/>',
	...
	CKEDITOR.plugins.add( 'rdimage', {
		...
		init: function( editor ) {
			...
			// when we'll rename rdimage to image.
			var config = editor.config,
				lang = editor.lang.rdimage,
				image = widgetDef( editor );

			// Since filebrowser plugin discovers config properties by dialog (plugin?)
			// names (sic!), this hack will be necessary as long as Rdimage is not named
			// Image. And since Rdimage will never be Image, for sure some filebrowser logic
			// got to be refined.
			config.filebrowserRdimageBrowseUrl = config.filebrowserImageBrowseUrl;
			config.filebrowserRdimageUploadUrl = config.filebrowserImageUploadUrl;
			...
			CKEDITOR.dialog.add( 'rdimage', this.path + 'dialogs/rdimage.js' );
		},
	...
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	// Non-captioned widget (config.rdimage_alignClasses defined)
	// 		┌──────┬───────────────────────────────┬─────────────────────────────┐
	// 		│Align │Internal form                  │Data                         │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│none  │<wrapper>                      │<img />                      │
	...
	// 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
	//
	// Captioned widget (config.rdimage_alignClasses defined)
	// 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
	// 		│Align │Internal form                           │Data                                    │
	// 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
	// 		│none  │<wrapper>                               │<figure />                              │
	...
	function widgetDef( editor ) {
		var alignClasses = editor.config.rdimage_alignClasses,
			captionedClass = editor.config.rdimage_captionedClass;
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.rdimage,
				...
				if ( editor.filter.checkFeature( this.features.dimension ) && editor.config.rdimage_disableResizer !== true && editor.readOnly != true ) {
					setupResizer( this );
				}
			...
			// Overrides default method to handle internal mutability of Rdimage.
			...
			getClasses: ( function() {
				...
				return function() {
					...
					// Neither config.rdimage_captionedClass nor config.rdimage_alignClasses
	...
	/**
	 * A set of Enhanced Image (rdimage) plugin helpers.
	 *
	 * @class
	 * @singleton
	 */
	CKEDITOR.plugins.rdimage = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.rdimage_alignClasses,
				captionedClass = editor.config.rdimage_captionedClass,
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.rdimage.captionPlaceholder
						} ), doc );
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						...
						if ( !newValue )
							...
							var attributes = CKEDITOR.plugins.rdimage.getLinkAttributesGetter()( editor, newValue );
	...
	// <figure class="{config.rdimage_captionedClass}"> elements.
	//
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// inline styles or classes (rdimage_alignClasses).
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.rdimage_captionedClass,
			alignClasses = editor.config.rdimage_alignClasses,
		...
		return function( el ) {
			...
			// Centering wrapper got to be... centering. If rdimage_alignClasses are defined,
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.rdimage.resizer );
			...
			if ( dialog.name == 'link' ) {
				...
				def.onShow = function() {
					...
					if ( widget && ( widget.inline ? !widget.wrapper.getAscendant( 'a' ) : 1 ) ) {
						...
						// Hide the display text in case of linking rdimage widget.
						displayTextField.hide();
					} else {
						// Make sure that display text is visible, as it might be hidden by rdimage integration
						// before.
	...
	// on configurations like config#rdimage_alignClasses or
	// config#rdimage_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {

        var classes = "rd-article-img-1,ccc";
		var alignClasses = editor.config.rdimage_alignClasses,
			rules = {
				...
				img: {
					attributes: '!src,alt,class,width,height',
                    classes: classes || null
				},
				figure: {
					classes: '!' + editor.config.rdimage_captionedClass
				},
				figcaption: true
			};
 ...
 *
 * Read more in the [documentation](#!/guide/dev_rdimage) and see the
 * [SDK sample](https://sdk.ckeditor.com/samples/rdimage.html).
 *
 *		// Changes the class to "captionedImage".
 *		config.rdimage_captionedClass = 'captionedImage';
 *
 * @cfg {String} [rdimage_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.rdimage_captionedClass = 'image';
 ...
 * [SDK sample](https://sdk.ckeditor.com/samples/rdimage.html).
 *
 *		config.rdimage_prefillDimensions = false;
 *
 * @since 4.5
 * @cfg {Boolean} [rdimage_prefillDimensions=true]
 * @member CKEDITOR.config
 */
 ...
 * [SDK sample](https://sdk.ckeditor.com/samples/rdimage.html).
 *
 *		config.rdimage_disableResizer = true;
 *
 * @since 4.5
 * @cfg {Boolean} [rdimage_disableResizer=false]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.rdimage_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4
 * @cfg {String[]} [rdimage_alignClasses=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.rdimage_altRequired = true;
 *
 * Read more in the [documentation](#!/guide/dev_rdimage) and see the
 * [SDK sample](https://sdk.ckeditor.com/samples/rdimage.html).
 *
 * @since 4.6.0
 * @cfg {Boolean} [rdimage_altRequired=false]
 * @member CKEDITOR.config
 */
