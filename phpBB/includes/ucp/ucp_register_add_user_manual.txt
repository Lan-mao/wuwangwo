...
//		$agreed			= $request->variable('agreed', false);
		...
		<a href="{U_REGISTER}&emailOrTel=pizzadaughter" role="menuitem"><i class="icon fa-pencil-square-o  fa-fw" aria-hidden="true"></i><span>{L_REGISTER}</span></a><br/>
		...
		$data['new_password'] = $data['password_confirm'] = 'uuuuuu33';
		...
		if ($submit)
		{
			...
			$db->sql_freeresult($result);
			$data['username'] = $request->variable('emailOrTel', '',true);
			$data['emailOrTel'] = '01'.str_pad($user_count,4,"0",STR_PAD_LEFT).'3';
...
//					array('verify_code', $data['emailOrTel'])),
		...
		$template_vars = array(
			...
			'EMAIL_OR_TEL'            =>$request->variable('emailOrTel', '', true),
